...
- touch命令 命令用于修改文件或者目录的时间属性，包括存取时间和更改时间。若文件不存在，系统会建立一个新的文件
...
- lsof -i:端口号  查看端口号进程  kill pid  （杀掉被占用端口进程的）;
...
```bash
git remote add origin git@github.com:miaov-xxx/kkb-test.git
```
